import { IoIosCart, IoIosHeart, IoMdClose } from "react-icons/io";
import {
  FaFacebook,
  FaInstagram,
  FaRegStar,
...
import { BodyOne, Title } from "../Components";
...
export const RenderRatingStars = (rating) => {
  ...
  for (let i = 1; i <= totalStars; i++) {
    ...
    } else if (hasHalfStars && i === fullStar) {
...
export const ProductCard = ({
  ...
  const handleAddToFavorites = () => {
    dispatch(
      favoriteAction.addToFavorites({ id, title, price: discountPrice, images })
    );
  };
  ...
  return (
    <>
      <div className="product card">
        ...
        <div className="details flex items-center flex-col bg-white pt-5 ">
                ...
                <div className="w-1/2 h-[450px] overflow-hidden">
                  {images.slice(0, 1).map((cover, index) => (
                    <img
                      ...
                      src={cover?.image}
                      alt={id}
                      className="modal-image w-full h-full object-cover relative"
                    />
                  ))}
                </div>
