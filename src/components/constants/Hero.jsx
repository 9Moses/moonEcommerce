...
import { BodyOne, Caption, Title } from "./Components";
...
function SamplePrevArrow(props) {
  ...
  return (
    <div
      className="absolute bottom-0 bg-black left-96 lg:left-[45.1%] slider-btn z-10"
      onClick={onClick}
    >
      <button className="next ">
...
export const Hero = () => {
  ...
  return (
    <>
      <section className="h-[50vh] lg:h-[90vh] mt-[60px] bg-white-100 relative z-1">
        <Slider {...settings}>
          {herolist.map((item) => (
            <HeroItem
              key={item.id}
  ...
  return (
    <>
      <section className="content flex justify-between lg:px-16 h-[50vh] lg:h-[90vh] relative z-20">
        <div className="left w-1/2 p-8 lg:p-22 lg:py-24 ">
          <Title
            level={1}
            className="leading-none font-medium md:text-3xl lg:text-[60px] lg:leading-snug"
          >
            {title}
          </Title>
        ...
        <div className="right bg-white p-5 w-1/2 flex justify-center items-center relative z-50">
        ...
        <div
          className={`${
            className
              ...
              : "flex absolute bottom-0 p-6 w-full"
